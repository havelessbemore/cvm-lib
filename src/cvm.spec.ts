...
describe(`${CVM.name}`, () => {
  ...
  describe(`.sampleRate`, () => {
    ...
    test("should handle edge case with zero probability gracefully", () => {
      ...
      expect(() => new CVM<number>(config)).toThrow(RangeError);
    });

    test("should handle edge case with one probability gracefully", () => {
      const config: CVMConfig = {
        capacity: 10,
        sampleRate: 1,
        randomFn: Math.random,
      };
  ...
  describe(`.randomFn`, () => {
    ...
    test("should return the custom random function if provided", () => {
      ...
      const customRandomFnCvm = new CVM<number>(config);
  ...
  describe(`.size`, () => {
    ...
    test("should correctly update size after hitting capacity and adjusting sample rate", () => {
      ...
      const cvm = new CVM<number>({ capacity: 3, randomFn: mockRandomFn });
      for (let i = 1; i <= 4; ++i) {
        cvm.add(i);
      }
  ...
  describe(`${CVM.prototype.add.name}()`, () => {
  ...
  describe(`${CVM.prototype.clear.name}()`, () => {
    test("should reset the sample rate to 1", () => {
      const cvm = new CVM<number>({ capacity: 2 });
      for (let i = 0; i < 4; ++i) {
        cvm.add(i);
      }
      expect((cvm as any)._rate).toBeLessThan(1);
